    ...
    watch: {
        ...
        selectBrand() {
            if (this.selectBrand.length && this.selectModel.length) {
                this.model = helpers.getCarModels(
                    this.product,
                    this.selectBrand,
                );
                return;
        ...
        selectModel() {
            if (this.selectModel.length && this.selectBrand.length) {
                ...
                this.brand = helpers.getUniqueElements(
                    this.product,
                    'brand',
                    true,
                    currentModel,
    ...
    computed: {
        ...
        filteringProduct() {
            return this.product
                .filter(
                    (product) =>
                        !this.selectBrand.length ||
                        this.selectBrand.includes(product.brand),
                )
                .filter(
                    (product) =>
                        !this.selectModel.length ||
                        this.selectModel.includes(product.model),
                )
                .filter(
                    (product) =>
                ...
                .filter(
                    (product) =>
                        !this.selectMinPrice ||
                        product.price >= this.selectMinPrice,
                )
                .filter(
                    (product) =>
                        !this.selectMaxPrice ||
                        product.price <= this.selectMaxPrice,
                )
                .filter(
                    (product) =>
                        !this.selectMinYear ||
                        product.year >= this.selectMinYear,
                )
                .filter(
                    (product) =>
                        !this.selectMaxYear ||
                        product.year <= this.selectMaxYear,
                );
        },
    },
};
